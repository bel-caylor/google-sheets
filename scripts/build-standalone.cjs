...
const replaceInclude = (buffer, name, content) => {
  const pattern = new RegExp(
    <\\?!=\\s*HtmlService\\.createHtmlOutputFromFile\\(''\\)\\.getContent\\(\\);?\\s*\\?>,
    'g'
  );
...
const ensureDistFile = (name) => {
  ...
  if (!fs.existsSync(filePath)) {
    throw new Error(Standalone build missing dist/.html. Run \"npm run build\" first.);
  }
  return filePath;
};
...
function main() {
  ...
  if (base) {
    const metaTag =   <meta name="app-script-base" content="">;
